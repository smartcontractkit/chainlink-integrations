...
type Store interface {
	...
	ForChain(chainID *big.Int) ChainStore
}
...
func (s *store) ForChain(chainID *big.Int) ChainStore {
	return &chainStore{store: s, chainID: chainID}
}
...
func NewChainStore(ks core.Keystore, chainID *big.Int) ChainStore {
	return NewStore(ks).ForChain(chainID)
}
